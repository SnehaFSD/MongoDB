...
const MONGO_URL = "mongodb://localhost:27017"//const MONGO_URL = "mongodb://127.0.0.127017"
...
app.get('/products/:id',async  (req, res) =>  {
    ...
    const product = await client.db("my-first-db").collection("products").findOne({id : id }) //in above connection is made in client so client is used
  ...
  app.delete('/products/:id',async  (req, res) =>  {
    ...
    console.log(req.params,id)
    //db.products.findOne({id : "1"}) // This is a mongodb query
    const product = await client.db("my-first-db").collection("products").deleteOne({id : id }) //in above connection is made in client so client is used
   // const product = products.find((pd) => pd.id === id)
    res.send(product) 
  })
...
//products?category=accessories&rating=4.5 =>filter accessories based on rating 4.5
